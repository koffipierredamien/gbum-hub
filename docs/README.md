# Les documents

**Commencez par [`../ETAT.md`](../ETAT.md)** : une page, tenue à jour.

| Fichier | Quand le lire |
|---|---|
| [Démarrer en local](11-DEMARRER-EN-LOCAL.md) | Faire tourner le projet sur sa machine |
| [Remplir le site](16-REMPLIR-LE-SITE.md) | Saisir les textes, les villes, les contacts |
| [Cahier des charges](01-CAHIER-DES-CHARGES.md) | Ce que le projet doit faire |
| [Architecture](02-ARCHITECTURE.md) | Pourquoi telle décision technique (les ADR) |
| [Note au Secrétariat National](07-NOTE-AU-SECRETARIAT-NATIONAL.md) | Ce qu'il faut leur demander |

## `archives/`

Douze documents écrits en cours de route : l'audit de l'existant, les
conventions de code, la direction artistique, les plans de lot, le cours de
prise en main, le catalogue de concepts, le guide de mise en ligne par serveur
loué.

Ils ne sont pas faux — ils sont **trop détaillés pour l'usage courant**. On y
revient quand une question précise se pose, pas avant.
